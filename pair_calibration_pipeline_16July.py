...
def extract_keypoints_pair2_518_536(images_pair2_518, images_pair2_536, opWrapper):
    """
    Extract keypoints and confidences for each image
    """

    images_cam1 = images_pair2_518
...
def extract_keypoints_pair3_536_520(images_pair3_536, images_pair3_520, opWrapper):
    """
    Extract keypoints and confidences for each image
    """

    images_cam1 = images_pair3_536
...
def filter_keypoints_pair2_518_536(keypoints_pair2_518, keypoints_pair2_536, threshold_pair2):
    """
    Filter keypoints based on the confidence score threshold and keep only the keypoints with matching IDs.
    ...
    Returns:
        np.ndarray, np.ndarray: Filtered keypoints arrays for keypoints1 and keypoints2 with matching IDs.
    """
    keypoints1 = keypoints_pair2_518
...
def filter_keypoints_pair3_536_520(keypoints_pair3_536, keypoints_pair3_520, threshold_pair3):
    """
    Filter keypoints based on the confidence score threshold and keep only the keypoints with matching IDs.
    ...
    Returns:
        np.ndarray, np.ndarray: Filtered keypoints arrays for keypoints1 and keypoints2 with matching IDs.
    """
    keypoints1 = keypoints_pair3_536
...
def triangulate_points_pair2_518_536(R_solvePnPRefineLM_pair2_518_536, tvec_refined_pair2_518_536, filtered_keypoints_pair2_518, filtered_keypoints_pair2_536):

    K1 = cameras[1]['matrix']
    K2 = cameras[2]['matrix']
    dist1 = cameras[1]['distortion']
    dist2 = cameras[2]['distortion']

    filtered_keypoints1 = filtered_keypoints_pair2_518
    filtered_keypoints2 = filtered_keypoints_pair2_536
    R_recoverPose = R_solvePnPRefineLM_pair2_518_536
    t_recoverPose = tvec_refined_pair2_518_536
...
def triangulate_points_pair3_536_520(R_solvePnPRefineLM_pair3_536_520, tvec_refined_pair3_536_520, filtered_keypoints_pair3_536, filtered_keypoints_pair3_520):

    K1 = cameras[2]['matrix']
    K2 = cameras[3]['matrix']
    dist1 = cameras[2]['distortion']
    dist2 = cameras[3]['distortion']

    filtered_keypoints1 = filtered_keypoints_pair3_536
    filtered_keypoints2 = filtered_keypoints_pair3_520
    R_recoverPose = R_solvePnPRefineLM_pair3_536_520
    t_recoverPose = tvec_refined_pair3_536_520
...
def estimate_solvePnPRefineLM_pair2_518_536(triangulated_points_pair1_518_536, filtered_keypoints_pair2_536, cameras):
    # Using OpenCV's solvePnPRefineLM function to estimate pose.
    ...
    points_3d = triangulated_points_pair1_518_536
    filtered_keypoints2 = filtered_keypoints_pair2_536
...
def estimate_solvePnPRefineLM_pair3_536_520( triangulated_points_pair2_518_536, filtered_keypoints_pair3_520, cameras):
    # Using OpenCV's solvePnPRefineLM function to estimate pose.
    ...
    points_3d = triangulated_points_pair2_518_536
    filtered_keypoints2 = filtered_keypoints_pair3_520
...
def calculate_reprojection_error(cam1, cam2, triangulated_points, filtered_keypoints2, R_solvePnP, tvec_solvePnP):
    K1 = cam1['matrix']
    K2 = cam2['matrix']
...
def calculate_reprojection_error_solvePnPRefine_pair2_518_536(triangulated_points_pair2_518_536, filtered_keypoints_pair2_536, R_solvePnPRefineLM_pair2_518_536, tvec_refined_pair2_518_536, cameras):

    K2 = cameras[2]['matrix']
    dist = cameras[2]['distortion']
    filtered_keypoints2 = filtered_keypoints_pair2_536
    triangulated_points = triangulated_points_pair2_518_536
    R_recoverPose = R_solvePnPRefineLM_pair2_518_536
    t_recoverPose = tvec_refined_pair2_518_536
...
if __name__ == '__main__':
    ...
    # Define the image pairs and their corresponding function names
    image_pairs = {
        "pair1_517_518": {
            "images": (images_pair1_517, images_pair1_518),
            "threshold": 0.7,
            "camera_params": (cameras[0], cameras[1])
        },
        "pair2_517_536": {
            "images": (images_pair2_517, images_pair2_536),
            "threshold": 0.7,
            "camera_params": (cameras[0], cameras[2])
        },
        "pair3_517_520": {
            "images": (images_pair3_517, images_pair3_520),
            "threshold": 0.7,
            "camera_params": (cameras[0], cameras[3])
        }
    }

    # # Initialize dictionaries to store the keypoints and confidences
    # keypoints = {}
    # confidences = {}
    # filtered_keypoints = {}
    # relative_positions = {}
    ...
    # for pair_name, pair_data in image_pairs.items():
    #     # Extract pair-specific data
    #     images1, images2 = pair_data["images"]
    #     threshold = pair_data["threshold"]
    #     cameras = pair_data["camera_params"]
    #
    #     # Call the function to extract keypoints
    #     keypoints_cam1, keypoints_cam2, confidences_cam1, confidences_cam2 = extract_keypoints(images1, images2,
    #                                                                                            opWrapper)
    #     keypoints[pair_name] = {"keypoints_cam1": keypoints_cam1, "keypoints_cam2": keypoints_cam2}
    #     confidences[pair_name] = {"confidences_cam1": confidences_cam1, "confidences_cam2": confidences_cam2}
    #
    #     # Apply filtering to the extracted keypoints with the specific threshold
    #     filtered_keypoints_cam1, filtered_keypoints_cam2 = filter_keypoints(keypoints_cam1, keypoints_cam2, threshold)
    #     filtered_keypoints[pair_name] = {"filtered_keypoints_cam1": filtered_keypoints_cam1,
    #                                      "filtered_keypoints_cam2": filtered_keypoints_cam2}
    #
    #     # Estimate relative position using the filtered keypoints
    #     R_recoverPose, t_recoverPose = estimate_RecoverPose(cameras, filtered_keypoints_cam1, filtered_keypoints_cam2)
    #     relative_positions[pair_name] = {"R_recoverPose": R_recoverPose, "t_recoverPose": t_recoverPose}
    #
    #     # Print the data for the pair
    #     print("Keypoints:")
    #     print(f"Camera 1: {keypoints[pair_name]['keypoints_cam1']}")
    #     print(f"Camera 2: {keypoints[pair_name]['keypoints_cam2']}")
    #     print("Confidences:")
    #     print(f"Camera 1: {confidences[pair_name]['confidences_cam1']}")
    #     print(f"Camera 2: {confidences[pair_name]['confidences_cam2']}")
    #     print("Filtered Keypoints:")
    #     print(f"Camera 1: {filtered_keypoints[pair_name]['filtered_keypoints_cam1']}")
    #     print(f"Camera 2: {filtered_keypoints[pair_name]['filtered_keypoints_cam2']}")
    #     print("Relative Position:")
    #     print(f"R_recoverPose: {relative_positions[pair_name]['R_recoverPose']}")
    #     print(f"t_recoverPose: {relative_positions[pair_name]['t_recoverPose']}")
    #     print("------------------------------")

        # Compute reprojection error
        # reproj_error = compute_reprojection_error(cameras, filtered_keypoints1, filtered_keypoints2, R_recoverPose,
        #                                             t_recoverPose)
        # print(f"Reprojection error for {pair_name}: {reproj_error}")
    ...
    # Compute reprojection error
    reproj_error_pair1 = calculate_reprojection_error(cam517, cam518, points_pair1, filtered_keypoints_pair1_518, R_recoverPose_pair1, t_recoverPose_pair1)
    reproj_error_pair2 = calculate_reprojection_error(cam517, cam536, points_pair2, filtered_keypoints_pair2_536, R_recoverPose_pair2, t_recoverPose_pair2)
    reproj_error_pair3 = calculate_reprojection_error(cam517, cam520, points_pair3, filtered_keypoints_pair3_520, R_recoverPose_pair3, t_recoverPose_pair3)
    ...
    # print('Keypoints extracted!', filtered_keypoints)
    # print('Recovered pose!', R_recoverPose, t_recoverPose)
    ...
    # keypoints_pair2_518, keypoints_pair2_536, confidences_pair2_518, confidences_pair2_536 = extract_keypoints_pair2_518_536(images_pair2_518, images_pair2_536, opWrapper)
    # keypoints_pair3_536, keypoints_pair3_520, confidences_pair3_536, confidences_pair3_520 = extract_keypoints_pair3_536_520(images_pair3_536,images_pair3_520, opWrapper)
    ...
    filtered_keypoints_pair2_518, filtered_keypoints_pair2_536 = filter_keypoints_pair2_518_536(keypoints_pair2_518, keypoints_pair2_536, threshold_pair2)
    filtered_keypoints_pair3_536, filtered_keypoints_pair3_520 = filter_keypoints_pair3_536_520(keypoints_pair3_536, keypoints_pair3_520, threshold_pair3)
    print(filtered_keypoints_pair2_536)
    print('Keypoints filtered for all pairs!')

    # RecoverPose between cam1 - cam2 and triangulation between cam1 - cam2
    R_recoverPose, t_recoverPose = estimate_RecoverPose(cameras, keypoints1, keypoints2)
    ...
    print(triangulated_points_pair1_517_518.shape, filtered_keypoints_pair2_536.shape,
          filtered_keypoints_pair2_518.shape)
    # SolvePnP between cam2 - cam3 and triangulation between cam2 - cam3
    R_solvePnPRefineLM_pair2_518_536, tvec_refined_pair2_518_536 = estimate_solvePnPRefineLM_pair2_518_536(triangulated_points_pair1_517_518, filtered_keypoints_pair2_518, cameras)

    triangulated_points_pair2_518_536 = triangulate_points_pair2_518_536(R_solvePnPRefineLM_pair2_518_536, tvec_refined_pair2_518_536, filtered_keypoints_pair2_518, filtered_keypoints_pair2_536)

    # SolvePnP between cam3 - cam4 and triangulation between cam3 - cam4
    R_solvePnPRefineLM_pair3_536_520, tvec_refined_pair3_536_520 = estimate_solvePnPRefineLM_pair3_536_520(triangulated_points_pair2_518_536, filtered_keypoints_pair3_520, cameras)
    triangulated_points_pair3_536_520 = triangulate_points_pair3_536_520(R_solvePnPRefineLM_pair3_536_520, tvec_refined_pair3_536_520, filtered_keypoints_pair3_536, filtered_keypoints_pair3_520)

    # Creating Extrinsic Matrix (4x4) for cams E12, E23, E34
    # Create a 4x4 identity matrix
    E12 = np.eye(4)
    E23 = np.eye(4)
    E34 = np.eye(4)

    # Insert R into the top left 3x3 block
    E12[:3, :3] = R_recoverPose_pair1_517_518
    E23[:3, :3] = R_solvePnPRefineLM_pair2_518_536
    E34[:3, :3] = R_solvePnPRefineLM_pair3_536_520
    # Insert T into the top right 3x1 block
    E12[:3, 3] = t_recoverPose_pair1_517_518
    E23[:3, 3] = tvec_refined_pair2_518_536
    E34[:3, 3] = tvec_refined_pair3_536_520
    ...
    estimated_distance_solvePnPRefine_pair2_518_536 = np.linalg.norm(tvec_refined_pair2_518_536)
